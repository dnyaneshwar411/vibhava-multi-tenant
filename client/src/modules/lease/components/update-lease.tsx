...
import { getUpdateLeaseDefaultValues } from "../config/update-lease-default-value";
import type { LeaseDetailsForForm, UpdateLeaseFormValues } from "../types";
import { updateLeaseSchema } from "../helpers/update-schema";

interface UpdateLeaseProps {
  lease: any;
...
export function UpdateLease({ lease, children }: UpdateLeaseProps) {
  const [open, setOpen] = useState(false);
  const defaultValues = getUpdateLeaseDefaultValues(lease);

  const form = useForm<UpdateLeaseFormValues>({
    resolver: zodResolver(updateLeaseSchema),
    defaultValues,
    mode: "all",
  });

  async function onSubmit(values: UpdateLeaseFormValues) {
    try {
      const payload = {
      ...
      const response = await api.put(`/api/v1/lease/${lease._id}`, {
        body: payload as any,
      });

      if (response.code !== 200) throw new Error(response.message);

      toast.success(response.message || "Lease updated");
      setOpen(false);
    ...
    >
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the lease record and its financial and security details.
          </DialogDescription>
        </DialogHeader>
